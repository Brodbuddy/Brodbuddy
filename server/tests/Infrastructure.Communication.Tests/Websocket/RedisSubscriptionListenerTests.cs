...
public class RedisSubscriptionListenerTests : IAsyncDisposable
{
    ...
    public class ProcessMessageAsync : RedisSubscriptionListenerTests
    {
        ...
        public async Task ProcessMessageAsync_SkipsUnavailableLocalSocket()
        {
            ...
            _socketManagerMock.Setup(m => m.TryGetLocalSocket(socketId, out It.Ref<IWebSocketConnection?>.IsAny))
                .Returns(
        ...
        public async Task ProcessMessageAsync_HandlesTopicWithNoLocalSubscribers()
        {
            ...
            _socketManagerMock
                .Setup(m => m.TryGetLocalSocket(It.IsAny<Guid>(), out It.Ref<IWebSocketConnection?>.IsAny))
                .Returns((Guid id, out IWebSocketConnection? socket) =>
                {
                    socket = null;
                    return false;
        ...
        public async Task ProcessMessage_HandlesTopicWithNoSubscribersInState()
        {
            ...
            _socketManagerMock.Verify(
                m => m.TryGetLocalSocket(It.IsAny<Guid>(), out It.Ref<IWebSocketConnection?>.IsAny),
                Times.Never);
        ...
        public async Task ProcessMessageAsync_ContinuesSending_WhenOneSocketSendThrows()
        {
            ...
            _socketManagerMock.Setup(m => m.TryGetLocalSocket(socketIdOk, out It.Ref<IWebSocketConnection?>.IsAny))
                .Returns(
                    (Guid id, out IWebSocketConnection? socket) =>
                    {
    ...
    public class StartAsyncTests : RedisSubscriptionListenerTests
    {
        [Fact]
        public async Task StartAsync_SubscribesToCorrectChannelPattern()
        {
            // Act
            ...
            _subscriberMock.Verify(s => s.SubscribeAsync(
                    It.Is<RedisChannel>(c => c.ToString() == RedisSocketKeys.AllTopicChannelsPattern()),
        ...
        [Fact]
        public async Task StartAsync_HandlesSubscriptionErrors()
        {
            // Arrange
            ...
            _redisMultiplexerMock.Verify(r => r.GetSubscriber(It.IsAny<object>()), Times.Once);
        }
    }

    public class StopAsyncTests : RedisSubscriptionListenerTests
    {
        [Fact]
        public async Task StopAsync_UnsubscribesFromAllChannels()
        {
            // Arrange
            await _listener.StartAsync(CancellationToken.None);
            ...
            // Assert
            _subscriberMock.Verify(s => s.UnsubscribeAllAsync(CommandFlags.None), Times.Once);
        }
        ...
        [Fact]
        public async Task StopAsync_HandlesUnsubscribeErrors()
        {
            // Arrange
            await _listener.StartAsync(CancellationToken.None);
            _subscriberMock.Setup(s => s.UnsubscribeAllAsync(It.IsAny<CommandFlags>()))
                           .ThrowsAsync(new RedisConnectionException(ConnectionFailureType.UnableToConnect, "Test exception"));
            ...
            // Assert
            _subscriberMock.Verify(s => s.UnsubscribeAllAsync(It.IsAny<CommandFlags>()), Times.Once, "Should attempt to unsubscribe even if it will fail");
        }
    }
