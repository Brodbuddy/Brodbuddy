...
public class IdentityVerificationServiceTests
{
    ...
    public class SendCodeAsync : IdentityVerificationServiceTests
    {
        ...
        public async Task SendCodeAsync_UserAlreadyHasRole_DoesNotAssignRole()
        {
            ...
            _userIdentityServiceMock.Setup(s => s.CreateAsync(TestEmail)).ReturnsAsync(TestUserId);
            _mockUserRoleService.Setup(s => s.GetUserRolesAsync(TestUserId)).ReturnsAsync(new List<Role> { existingRole });
            _otpServiceMock.Setup(s => s.GenerateAsync()).ReturnsAsync((TestOtpId, TestCode));
            _repositoryMock.Setup(r => r.CreateAsync(TestUserId, TestOtpId)).ReturnsAsync(Guid.NewGuid());

            _transactionManagerMock
                .Setup(tm => tm.ExecuteInTransactionAsync(It.IsAny<Func<Task<bool>>>()))
                .Returns<Func<Task<bool>>>(async operation => await operation());
