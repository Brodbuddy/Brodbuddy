...
namespace TimeUtils {
    ...
    inline void delay_for(const duration<Rep, Period>& d) {
        auto ms = duration_cast<milliseconds>(d);
        delay(ms.count());
    }
    
    template<typename Rep, typename Period>
    inline void enable_sleep_timer(const duration<Rep, Period>& d) {
        auto us = duration_cast<microseconds>(d);
        esp_sleep_enable_timer_wakeup(us.count());
    }
    
    template<typename Rep, typename Period>
    inline unsigned long to_ms(const duration<Rep, Period>& d) {
        return duration_cast<milliseconds>(d).count();
    }
    
    template<typename Rep, typename Period>
    inline uint64_t to_us(const duration<Rep, Period>& d) {
        return duration_cast<microseconds>(d).count();
    }
    
    template<typename Rep, typename Period>
    inline int to_seconds(const duration<Rep, Period>& d) {
