...
void setup() {
    ...
    LOG_I(TAG, "VL53L0X Sensor Test Starting");
    ...
    // Initialize sensor
    if (sensor.init()) {
        LOG_I(TAG, "VL53L0X sensor initialized successfully");
        sensor.setTimeout(TimeUtils::to_ms(TimeConstants::VL53L0X_TIMEOUT));
        sensor.setMeasurementTimingBudget(TimeUtils::to_us(TimeConstants::VL53L0X_TIMING_BUDGET));
        sensor.startContinuous();
        TimeUtils::delay_for(TimeConstants::VL53L0X_STABILIZATION_DELAY);
        LOG_I(TAG, "VL53L0X sensor ready for readings");
    } else {
        LOG_E(TAG, "Failed to initialize VL53L0X sensor");
    }
}

void loop() {
    if (sensor.timeoutOccurred()) {
        LOG_E(TAG, "Sensor timeout occurred");
        TimeUtils::delay_for(1s);
        return;
    }
    
    uint16_t distance = sensor.readRangeContinuousMillimeters();
    
    if (distance >= Sensors::TOF_DISTANCE_MIN && distance <= Sensors::TOF_DISTANCE_MAX) {
        LOG_I(TAG, "Distance: %d mm", distance);
    } else {
        LOG_W(TAG, "Distance out of range: %d mm", distance);
    }
    
    TimeUtils::delay_for(500ms);
}
